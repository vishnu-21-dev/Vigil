...
print(f"Targeting device: {devices[0]['name']} ({target_device})")
...
# Create a payload with wildly extreme out-of-distribution values to guarantee an anomaly detection
payload = {
    "device_id": target_device,
    "features": {f: 999999.9 for f in features}
}

response = requests.post("http://localhost:8000/monitor/ingest", json=payload)
